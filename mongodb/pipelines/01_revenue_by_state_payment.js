...
// Pipeline complejo: $match, $lookup, $unwind, $addFields, $group, $project, $sort, $facet.
...
[
  ...
  { $unwind: { path: "$reviews", preserveNullAndEmptyArrays: true } },
  {
    $addFields: {
      ...
      review_score_safe: { $ifNull: ["$reviews.review_score", 0] }
    }
  },
  {
    $group: {
      ...
      total_orders: { $sum: 1 },
      total_revenue: { $sum: "$payment_summary.total_value" },
  ...
  { $sort: { total_revenue: -1 } },
  {
    $facet: {
      top_states: [{ $limit: 10 }],
      summary: [
        { $group: { _id: null, total_revenue: { $sum: "$total_revenue" }, total_orders: { $sum: "$total_orders" } } }
      ]
    }
  }
],
